...
export const genarateAiContent = async(req : Request, res: Response) => {
    ...
    const createLearningPathResponse = await axios.post("https://openrouter.ai/api/v1/chat/completions",
        {
            model: "gpt-4o-mini",
            messages: [
                {
    ...
    try {
      learningPath = parse(learningPathRaw);
      console.log("Learning path parsed successfully");
    } catch (parseError) {
      console.warn(
        "Failed to parse AI JSON, sending raw string instead:", parseError
      );
...
export const genaratePerfectAnswers = async (req: Request, res: Response) => {
...
function buildLeraningPath(subject: string) {
    return `
...
function genarateNoteContent(subject: string, topics: string[]) {
    return `
...
function genarateAnswers(subject: string, questions: string[]) {
  return `
